...
def pos_def(text):
    doc = nlp(text)
    # return a SINGLE POS tag, not combined tags
    return [t.pos_ for t in doc]

# Apply POS extraction
df_pos = df["word"].apply(pos_def)

# Flatten list of lists into one list of POS tags
pos = [tag for sublist in df_pos for tag in sublist]
...
fig_100 = px.scatter(
    ...
    title="Top 100 Word Pairs (Interactive)")
fig_100.show()

fig_100.write_html("100_arm_wup_plot.html", include_plotlyjs="cdn")
```
...
wup_df["pair"] = wup_df["w1"] + " – " + wup_df["w2"]
...
fig.write_html("arm_wup_plot.html", include_plotlyjs="cdn")
```
